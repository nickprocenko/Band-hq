...
create policy "enable_read_chord_charts"
  on public.chord_charts for select using (true);

create policy "enable_write_chord_charts"
  on public.chord_charts for all using (true) with check (true);

-- Link setlist songs to saved chord charts (nullable, set-null on delete)
